...
public class SDViewPageIndicator extends HorizontalScrollView implements OnPageChangeListener
{
    ...
    public void setAdapter(SDAdapter adapter)
    {
        if (mAdapter != adapter)
        {
            if (mAdapter != null)
            {
                mAdapter.unregisterDataSetObserver(defaultDataSetObserver);
            }
            mAdapter = adapter;
            if (adapter != null)
            {
                adapter.registerDataSetObserver(defaultDataSetObserver);
            }
            notifyDataSetChanged();
        }
    }

    private DataSetObserver defaultDataSetObserver = new DataSetObserver()
    {
        @Override
