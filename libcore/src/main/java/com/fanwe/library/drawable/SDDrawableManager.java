...
public class SDDrawableManager
{
	...
	public SDDrawableManager()
	{
		...
		if (config != null)
		{
			setMainColor(config.getColorMain());
			setMainColorPressed(config.getColorMainPress());
			setStrokeColor(config.getColorStroke());
			setGrayColorPressed(config.getColorGrayPress());
			setCorner(config.getCorner());
			setStrokeWidth(config.getWidthStroke());
		}
	}
